...
LOSS_FUNCTION_NAME = "weighted_high_demand_huber"
  
PERMUTATION_SAMPLE_SIZE = 500
RUN_PERMUTATION_IMPORTANCE = False
RUN_NAME = "lstm_Toan_encoder_decoder_weighted_high_demand_huber"
TARGET_COL = "Demand"
...
def build_lstm_model(input_shape):
    ...
    model.compile(
        optimizer=optimizer,
        loss=weighted_high_demand_huber_loss,
        metrics=["mae"],
    )
...
# y_true and y_pred are scaled by MinMaxScaler, so high demand is closer to 1.
# Higher actual demand receives a higher loss weight to focus learning on peaks.
#=================================================
def weighted_high_demand_huber_loss(y_true, y_pred):
    error = y_true - y_pred
    ...
    weights = 1.0 + 3.0 * tf.square(y_true)
