...
MODEL_PATH = cfg.MODEL_DIR / "tft_peak_aware_asymmetric_mse_model.keras"
...
LOSS_FUNCTION_NAME = "peak_aware_asymmetric_mse"
PEAK_THRESHOLD = 0.75
PEAK_WEIGHT = 3.0
UNDER_PREDICTION_WEIGHT = 2.0
OVER_PREDICTION_WEIGHT = 1.0

RUN_NAME = (
    f"TFT_Toan_July_18_{LOSS_FUNCTION_NAME}"
    f"_peak{PEAK_THRESHOLD}_peak_weight{PEAK_WEIGHT}"
    f"_under_weight{UNDER_PREDICTION_WEIGHT}"
)
USE_MLFLOW = True
TARGET_COL = "Demand"
TIME_COL = "datetime"
...
def peak_aware_asymmetric_mse(y_true, y_pred):
    peak_weights = tf.where(
        y_true >= PEAK_THRESHOLD,
        tf.cast(PEAK_WEIGHT, y_true.dtype),
        tf.cast(1.0, y_true.dtype),
    )
    direction_weights = tf.where(
        y_pred < y_true,
        tf.cast(UNDER_PREDICTION_WEIGHT, y_true.dtype),
        tf.cast(OVER_PREDICTION_WEIGHT, y_true.dtype),
    )
    squared_error = tf.square(y_true - y_pred)
    return tf.reduce_mean(peak_weights * direction_weights * squared_error)
...
def build_tft_model():
    ...
    model.compile(
        optimizer=optimizer,
        loss=peak_aware_asymmetric_mse,
        metrics=["mae"],
    )
...
def save_training_loss_plot(history, output_path):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Peak-Aware Asymmetric MSE")
    plt.tight_layout()
...
def save_first_forecast_plot(
    ...
    plt.legend()
    plt.title(
        f"First Test Sample: {FORECAST_HORIZON}-Hour TFT "
        "Peak-Aware Asymmetric MSE Forecast"
    )
...
def save_first_forecast_block_plots(
    ...
    for block_idx, start_idx in enumerate(range(0, FORECAST_HORIZON, block_size), start=1):
        ...
        output_path = (
            output_dir
            / f"tft_peak_aware_asymmetric_mse_first_24h_block_{block_idx}_forecast.png"
        )
...
def log_mlflow_run_metadata(df, train_meta, val_meta, test_meta, X_train, X_val, X_test):
    params = {
        ...
        "over_prediction_weight": OVER_PREDICTION_WEIGHT,
...
def train_tft():
    ...
    with run_context:
        ...
        training_loss_plot_path = PLOT_DIR / "tft_peak_aware_asymmetric_mse_training_loss.png"
        forecast_plot_path = (
            PLOT_DIR / "tft_peak_aware_asymmetric_mse_test_actual_vs_forecast.png"
        )
        first_forecast_plot_path = (
            PLOT_DIR / f"tft_peak_aware_asymmetric_mse_first_{FORECAST_HORIZON}h_forecast.png"
        )
        horizon_mae_plot_path = PLOT_DIR / "tft_peak_aware_asymmetric_mse_horizon_mae.png"
        horizon_mae_csv_path = PLOT_DIR / "tft_peak_aware_asymmetric_mse_horizon_mae.csv"
        block_24h_metrics_csv_path = (
            PLOT_DIR / "tft_peak_aware_asymmetric_mse_24h_block_metrics.csv"
        )
