...
PEAK_LOSS_ALPHA = 3.0
LOSS_FUNCTION_NAME = f"weighted_quantile_loss_alpha_{PEAK_LOSS_ALPHA}"
...
def weighted_quantile_loss(y_true, y_pred):
    quantiles = tf.constant(QUANTILES, dtype=y_pred.dtype)
    y_true = tf.cast(y_true, y_pred.dtype)
    error = y_true - y_pred
    quantile_errors = tf.maximum(quantiles * error, (quantiles - 1.0) * error)

    normalized_actual = tf.clip_by_value(
        y_true,
        clip_value_min=0.0,
        clip_value_max=1.5,
    )
    peak_weight = 1.0 + PEAK_LOSS_ALPHA * tf.square(normalized_actual)
    weighted_errors = peak_weight * quantile_errors
...
def log_mlflow_run_metadata(df, train_meta, val_meta, test_meta, X_train, X_val, X_test):
    params = {
        ...
        "peak_loss_alpha": PEAK_LOSS_ALPHA,
        "peak_weight_formula": "1 + alpha * scaled_actual^2",
